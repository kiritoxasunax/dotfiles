...
# System maintenance
alias sysup="sudo ~/bin/system-maintenance"
source ~/powerlevel10k/powerlevel10k.zsh-theme

# To customize prompt, run `p10k configure` or edit ~/.p10k.zsh.
[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh

# Mount darla /storage/data via sshfs (uses ~/.ssh/config)
mount-darla() {
  local mnt="$HOME/mnt/darla-storage"
  mkdir -p "$mnt"
  if command -v mountpoint >/dev/null 2>&1 && mountpoint -q "$mnt"; then
    echo "Already mounted: $mnt"
    return 0
  fi
  sshfs -o reconnect,ServerAliveInterval=15,ServerAliveCountMax=3,Compression=yes,idmap=user \
    darla:/storage/data \
    "$mnt"
}
